...
import { handleAfternoonCheck, handleEveningCheck } from '@/lib/maia-handler'
...
export async function POST(request: Request): Promise<Response> {
  ...
  setImmediate(() => {
    const fn = slot === 'evening' ? handleEveningCheck : handleAfternoonCheck
    fn(channel).catch((err: unknown) =>
      console.error(`[cron/maia/daily-check/${slot}] error:`, err),
    )
  })
