...
DEMETER's second surface: clicking her widget in the MAIA dashboard opens a Bloomberg-style terminal. **Combo data model:**

- **Research (any asset):** embed free **TradingView widgets** — Symbol Search, Advanced Chart, Top Stories (news), Symbol Info. They cover stocks, indices, ETFs, crypto and forex with charts + news out of the box — no backend data pipeline, no cost. This is the "search and research anything" half.
- **My Book (what you hold):** rendered from your own `holdings` data + `tools/market-data.ts` quotes → live value, day P&L, total P&L, allocation. This is the accurate-numbers half.
...
- **Research tab:** a large advanced chart for the selected symbol with timeframe controls; a side panel of key stats + a news feed.
- **My Book tab:** portfolio summary (total value, day P&L, total P&L) + holdings table (qty, price, value, day %, P&L, allocation bar) + watchlist.
- Selecting any symbol — from search, ticker tape, watchlist, or a holding row — loads it in the Research chart.
...
**Build note:** the terminal is a **follow-on** to the backend brief — build holdings + the scheduled push first (steps 1–4 below), then layer the terminal on top using the same holdings data. TradingView widgets are frontend-only embeds, so they add no server load and need no API keys.
