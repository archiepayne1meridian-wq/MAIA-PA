...
| OpenBB | DEMETER terminal data (quotes, history, news) | Bearer token | `OPENBB_URL`, `OPENBB_TOKEN` (MAIA); `OPENBB_API_KEY` (service) | Active — D3a complete |
| FMP | OpenBB FX cross-check (yfinance covers equities; FMP free tier = FX only) | API key | `FMP_API_KEY` in openbb-service env | Active — FX only on free tier |
